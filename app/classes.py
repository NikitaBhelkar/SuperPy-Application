...
import csv
from config import bought_file,bought_file_copy,sold_file,artificial_date,products_expired
import pandas as pd
import sys
import shutil
...
class Product:
    def __init__(self, product_name, buy_price, expiration_date):
         # casefold() is used to ignore case for product name
         self.product_name = product_name.casefold()
...
class ProductSell:
    ...
    def write_to_soldcsvfile(self):
        ...
        try:
            ...
            # filter products from dataset by productname and those which are not expired
