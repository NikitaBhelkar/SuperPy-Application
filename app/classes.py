...
import csv
from config import bought_file,bought_file_copy,sold_file,products_instock_aftersell,artificial_date
import pandas as pd
import sys
import shutil
...
class ProductSell:
    ...
    def write_to_soldcsvfile(self):
        ...
        try:
            ...
            print(df)
            """check if product_name is present in bought products file and expiration_date is less and today"""
            with open(artificial_date, 'r', newline='') as f:
            ...
            df = df.loc[df['expiration_date'] < expiry_date_value]
            print(df)
            with open(bought_file_copy, 'r') as fin:
                 ...
                 for row in reader:
                     ...
                     if (row and (row[1] == 'orange') and (row[4] <= expiry_date_value)):
                         bought_id = row[0]
